 ...
 * Usage:
 *   node scripts/aggregate-benchmark.cjs --trials <trials.json> --artifact <path> [--prior <benchmark.json>]
 *                                        [--min-pass-rate 0.8] [--max-flaky-std 0.2] [--max-regression 0.1]
 *
...
const evals = trials.evals.map((ev) => {
  ...
  const deltaMean =
    withStats.pass_rate_mean !== null && baseStats.pass_rate_mean !== null
